...
if(trim($casenum) != ''){
    $query .= " AND  `case`.casenum='$casenum'";}
}

if (!empty($_POST['year'])) {
	$year=$_POST['year'];
if(trim($year) != ''){
    $query .= " AND  `case`.caseyear='$year'";
}}

if (!empty($_POST['sarkiid'])) {
	$sarkiid=$_POST['sarkiid'];
if(trim($sarkiid) != ''){
    $query .= " AND  `sarki`.idsarki='$sarkiid'";
}}

if (!empty($_POST['type'])) {
	$type=$_POST['type'];
if(trim($type) != ''){
    $query .= " AND  casetype.idcasetype='$type'";
}}

if (!empty($_POST['depart'])) {
	$depart=$_POST['depart'];
if(trim($depart) != ''){
    $query .= " AND  departs.iddeparts='$depart'";
}}

if (!empty($_POST['from'])) {
	$from=$_POST['from'];
if(trim($from) != ''){
    $query .= " AND  `case`.createdate >= '$from'";
}}

if (!empty($_POST['to'])) {
	$to=$_POST['to'];
if(trim($to) != ''){
    $query .= " AND  `case`.createdate <= '$to 23:59:59'";
}}	

if (!empty($_POST['user'])) {
	$user=$_POST['user'];
if(trim($user) != ''){
    $query .= " AND  `users`.idusers = '$user'";
}}
											...
											<div class="form-group">
												<label class="col-sm-1 control-label no-padding-right" for="form-field-4"> مدخل البيانات </label>
												<div class="col-sm-8">
												...
												<br>
												<br>
												<label class="col-sm-1 control-label no-padding-right" for="form-field-4"> رقم السركي </label>
												<div class="col-sm-8">
													<input type="text" value="<?php if (!empty($sarkiid)) { echo $sarkiid; }?>" class="input-sm" id="spinner5" name="sarkiid"/>
												</div>
												<br>
												<br>
										...
										<div>
											<table id="dynamic-table" class="table table-striped table-bordered table-hover">
												...
												<tbody>
													...
													?>
													<tr>
														<td><a class="green" href="sarkiprofile.php?idsarki=<?php echo $row4['idsarki'] ?>"><?php echo $row4['idsarki'] ?></a></td>
